...
func (service serviceApp) LoginFresh(_ context.Context) (response domainApp.LoginResponse, err error) {
	if service.WaCli == nil {
		return response, pkgError.ErrWaCLI
	}
	...
	// Clear any existing QR code files
		...
		for evt := range ch {
			...
			if evt.Event == "code" {
				...
				go func() {
					...
					err := os.Remove(qrPath)
					if err != nil {
						logrus.Error("error when remove fresh qrImage file", err.Error())
					}
				}()
				chImage <- qrPath
				logrus.Info("Fresh QR code generated: ", qrPath)
			} else {
				logrus.Error("error when get fresh qrCode", evt.Event)
			}
		}
	}()
	...
	err = service.WaCli.Connect()
	if err != nil {
		logger.Error("Error when connect to whatsapp for fresh login", err)
		return response, pkgError.ErrReconnect
	}
	response.ImagePath = <-chImage
