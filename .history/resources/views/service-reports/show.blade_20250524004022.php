...
<body class="bg-gray-50 font-sans">
    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div
            class="bg-gradient-to-br from-primary-50 to-primary-100 rounded-lg shadow-lg p-8 mb-8 border-2 border-primary-200">
            <div class="text-center mb-6">
                <div
                    class="inline-flex items-center justify-center w-16 h-16 bg-primary-600 text-white rounded-full mb-4">
                ...
                <h3 class="text-2xl font-bold text-primary-800 mb-2">E-Certificate Mobil Sehat</h3>
                <p class="text-primary-700 font-medium">by Hartono Motor</p>
            </div>

            <!-- Certificate Content -->
            <div class="bg-white rounded-lg border-4 border-primary-300 p-6 shadow-inner">
                <!-- Certificate Header -->
                <div class="text-center border-b-2 border-primary-200 pb-4 mb-6">
                    <div class="flex justify-center items-center mb-4">
                        <img src="{{ asset('images/logo/logo.png') }}" alt="Hartono Motor" class="h-16 mr-4">
                        <div>
                            <h4 class="text-xl font-bold text-primary-800">HARTONO MOTOR</h4>
                            <p class="text-sm text-primary-600">Bengkel Mobil Terpercaya</p>
                        </div>
                    </div>
                    <h5 class="text-lg font-semibold text-gray-800">SERTIFIKAT KESEHATAN KENDARAAN</h5>
                    <p class="text-sm text-gray-600">Vehicle Health Certificate</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="font-mono font-bold text-primary-800">{{ $report->certificate_number }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Nama Pemilik:</p>
                            <p class="font-semibold text-gray-800">{{ $report->customer_name }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Nomor Plat:</p>
                            <p class="font-semibold text-gray-800">{{ $report->license_plate }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Model Kendaraan:</p>
                            <p class="font-semibold text-gray-800">{{ $report->car_model }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            <p class="text-sm font-medium text-gray-500">Tanggal Pemeriksaan:</p>
                            <p class="font-semibold text-gray-800">{{ $report->certificate_issued_date->format('d F Y')
                                }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Berlaku Hingga:</p>
                            <p class="font-semibold text-gray-800">{{ $report->certificate_valid_until->format('d F Y')
                                }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Teknisi:</p>
                            <p class="font-semibold text-gray-800">{{ $report->technician_name ?? 'Tim Hartono Motor' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">Kode Verifikasi:</p>
                            <p class="font-mono font-bold text-primary-800">{{ $report->certificate_verification_code }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center bg-gray-50 rounded-lg p-6 mb-6">
                    <p class="text-sm font-medium text-gray-500 mb-2">Status Kesehatan Kendaraan:</p>
                    <div class="flex justify-center items-center space-x-4">
                        <div class="text-center">
                            <p class="text-3xl font-bold {{ $report->getHealthStatusColor() }}">{{
                                $report->health_status }}</p>
                            <p class="text-sm text-gray-600">Kondisi Umum</p>
                        </div>
                        <div class="text-center">
                            <p class="text-3xl font-bold text-primary-600">{{ $report->overall_condition_score }}%</p>
                            <p class="text-sm text-gray-600">Skor Kondisi</p>
                        </div>
                    </div>
                </div>

                <!-- Certificate Statement -->
                <div class="text-center border-t-2 border-primary-200 pt-4">
                    <p class="text-sm text-gray-700 leading-relaxed">
                        Berdasarkan pemeriksaan menyeluruh yang telah dilakukan, kendaraan dengan nomor plat
                        <strong>{{ $report->license_plate }}</strong> telah dinyatakan dalam kondisi
                        <strong class="{{ $report->getHealthStatusColor() }}">{{ $report->health_status }}</strong>
                        dengan skor kondisi <strong>{{ $report->overall_condition_score }}%</strong>.
                    </p>
                    <div class="mt-4 flex justify-between items-end">
                        <div class="text-left">
                            <p class="text-xs text-gray-500">Diterbitkan secara digital oleh:</p>
                            <p class="font-semibold text-primary-800">Hartono Motor</p>
                            <p class="text-xs text-gray-600">Jl. Samanhudi No 2, Kebonsari, Sidoarjo</p>
                        </div>
                        <div class="text-right">
                            <div
                                class="w-20 h-20 bg-primary-100 rounded-lg flex items-center justify-center border-2 border-primary-300">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-primary-600" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">Digital Seal</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center mt-6">
                <a href="{{ route('service-reports.certificate', $report->code) }}"
                    class="inline-flex items-center px-6 py-3 bg-primary-600 text-white font-semibold rounded-lg hover:bg-primary-700 transition shadow-lg">
                ...
                <p class="text-xs text-gray-600 mt-2">Unduh sertifikat dalam format PDF untuk disimpan atau dicetak</p>
            </div>
        </div>
