        ...
        <div class="space-y-4">
            ...
            <div class="flex space-x-2 mb-4">
                <a href="?status=completed"
                    class="px-4 py-2 text-sm font-medium rounded-md {{ $status === 'completed' ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300' }}">
                    Selesai
                </a>
                <a href="?status=in_progress"
                    class="px-4 py-2 text-sm font-medium rounded-md {{ $status === 'in_progress' ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300' }}">
                    Dalam Pengerjaan
                </a>
                <a href="?status=cancelled"
                    class="px-4 py-2 text-sm font-medium rounded-md {{ $status === 'cancelled' ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300' }}">
                    Dibatalkan
                </a>
                <a href="?status=all"
                    class="px-4 py-2 text-sm font-medium rounded-md {{ $status === 'all' ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300' }}">
                    Semua Status
                </a>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr class="bg-gray-50">
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nomor Nota</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Biaya Jasa</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $service->service_type }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $service->invoice_number }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Rp {{ number_format($service->labor_cost, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    @if($service->status == 'completed')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Selesai</span>
                                    @elseif($service->status == 'in_progress')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Dalam Pengerjaan</span>
                                    @elseif($service->status == 'cancelled')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Dibatalkan</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $service->created_at->format('d M Y H:i') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $service->completed_at ? $service->completed_at->format('d M Y H:i') : '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <a href="{{ route('filament.admin.resources.services.edit', ['record' => $service->id]) }}" class="text-blue-600 hover:text-blue-900">Lihat Detail</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
