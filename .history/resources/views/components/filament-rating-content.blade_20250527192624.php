        ...
        <div>
            ...
            <div class="flex space-x-1" id="stars-{{ $mechanic->id }}">
                @for($i = 1; $i <= 5; $i++) <button type="button"
                    ...
                    class="text-gray-300 hover:text-yellow-400 transition-colors duration-200 cursor-pointer"
                    id="star-{{ $mechanic->id }}-{{ $i }}" style="background: none; border: none; padding: 2px;">
...
<script>
    // Enhanced rating system for Filament modal with detailed debugging
    let ratings = {};

    function setRating(mechanicId, rating) {
        console.log(`🎯 setRating called: mechanicId=${mechanicId}, rating=${rating}`);

        // Store rating
        ratings[mechanicId] = rating;
        console.log(`✅ Rating stored:`, ratings);
    ...
    function submitRating(serviceId, mechanicId) {
        const rating = ratings[mechanicId];
        if (!rating) {
