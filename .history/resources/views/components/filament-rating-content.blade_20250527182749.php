...
<div class="space-y-4">
    ...
    <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $service->license_plate }} - {{ $service->car_model }}</p>
    ...
    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
        ...
        <div class="flex justify-between items-center">
            ...
            <span id="status-{{ $mechanic->id }}" class="text-xs px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                Belum Rating
            </span>
        </div>
        ...
        <div>
            ...
            <div class="flex space-x-1" id="stars-{{ $mechanic->id }}">
                @for($i = 1; $i <= 5; $i++)
                <button type="button" 
                        onclick="setRating({{ $mechanic->id }}, {{ $i }})"
        ...
        <!-- Submit Button -->
        <button type="button" 
                id="submit-{{ $mechanic->id }}"
                onclick="submitRating({{ $service->id }}, {{ $mechanic->id }})"
                disabled
...
<script>
// Simple rating system for Filament modal
let ratings = {};
...
function setRating(mechanicId, rating) {
    ...
    // Update star colors
    for (let i = 1; i <= 5; i++) {
        const star = document.getElementById(`star-${mechanicId}-${i}`);
        if (i <= rating) {
    ...
    // Update status
    const status = document.getElementById(`status-${mechanicId}`);
