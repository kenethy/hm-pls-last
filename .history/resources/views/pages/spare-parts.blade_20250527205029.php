...
<section id="kategori" class="py-16 bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            ...
            <a href="{{ route('spare-parts.category', $category->slug) }}"
                ...
                <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 transition-colors"
                    style="background-color: {{ $category->color ?? '#dc2626' }}20; color: {{ $category->color ?? '#dc2626' }};">
                    @php
                    // Define default icons for common categories
                    $defaultIcons = [
                    'oli-cairan' => 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0
                    01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367
                    3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z',
                    'sistem-rem' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732
                    4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
                    'transmisi' => 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0
                    01-15.357-2m15.357 2H15',
                    'sistem-pengapian' => 'M13 10V3L4 14h7v7l9-11h-7z',
                    'filter' => 'M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0
                    00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.414A1 1 0 013 6.707V4z',
                    'aki-battery' => 'M13 10V3L4 14h7v7l9-11h-7z',
                    'suspensi' => 'M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4',
                    'radiator' => 'M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0
                    00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z',
                    'aksesoris' => 'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5
                    12l5.714-2.143L13 3z',
                    'default' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0
                    00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'
                    ];

                    $iconPath = $defaultIcons[$category->slug] ?? $defaultIcons['default'];
                    @endphp

                    @if($category->icon && str_contains($category->icon, 'heroicon'))
                    {{-- Heroicon class --}}
                    <i class="{{ $category->icon }} h-8 w-8"></i>
                    @elseif($category->icon && str_contains($category->icon, '<svg')) {{-- Custom SVG --}} {!!
                        $category->icon !!}
                        @else
                        {{-- Default SVG based on category slug --}}
                ...
                <h3 class="font-bold text-lg group-hover:text-red-600 transition-colors">{{ $category->name }}</h3>
                <p class="text-sm text-gray-500 mt-2">{{ Str::limit($category->description, 30) }}</p>
                @if($category->spare_parts_count > 0)
                <span class="inline-block mt-2 bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full">
                    {{ $category->spare_parts_count }} produk
                </span>
                @endif
            </a>
            @empty
