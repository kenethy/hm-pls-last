...
class UpdateMechanicReports
{
    ...
    /**
     * Handle the ServiceStatusChanged event. what
     */
        ...
        DB::transaction(function () use ($service, $mechanics) {
            ...
            foreach ($mechanics as $mechanic) {
                try {
                    ...
                    Log::info("UpdateMechanicReports: Updating cumulative report for mechanic #{$mechanic->id} with labor cost {$laborCost}");

                    // Update cumulative report for this mechanic
                    $this->updateCumulativeReport($mechanic, $service->id, $laborCost);
                } catch (\Exception $e) {
    ...
    /**
     * Update cumulative report for a mechanic.
     */
    private function updateCumulativeReport(Mechanic $mechanic, int $serviceId, float $laborCost): void
    {
        try {
            Log::info("UpdateMechanicReports: Updating cumulative report for mechanic #{$mechanic->id}");
            ...
            Log::info("UpdateMechanicReports: Successfully updated cumulative report for mechanic #{$mechanic->id}", [
                'report_id' => $report->id,
                'services_count' => $report->services_count,
                'total_labor_cost' => $report->total_labor_cost,
            ]);
        } catch (\Exception $e) {
            Log::error("UpdateMechanicReports: Error updating cumulative report for mechanic #{$mechanic->id}", [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
        }
