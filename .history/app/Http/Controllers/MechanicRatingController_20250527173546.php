    ...
    public function checkRatingPopup(Request $request): JsonResponse
    {
        try {
            ...
            if (session('trigger_rating_modal') && session('current_rating_service')) {
                ...
                session()->forget(['trigger_rating_modal', 'current_rating_service']);
            }
            // Check for bulk ratings
            else if (session('pending_bulk_ratings')) {
                $response['bulk_ratings'] = session('pending_bulk_ratings');
                session()->forget(['pending_bulk_ratings']);
            }
            // Legacy support for direct modal trigger
            else if (session('show_rating_modal_direct') && session('pending_rating_service')) {
                $response['show_direct_modal'] = true;
                $response['service_data'] = session('pending_rating_service');

                // Clear the session flags after reading
                session()->forget(['show_rating_modal_direct', 'pending_rating_service']);
            }
            // Legacy support for popup trigger (fallback)
            else if (session('show_rating_popup') && session('pending_rating_service')) {
