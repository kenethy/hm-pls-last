    ...
    public function checkRatingPopup(Request $request): JsonResponse
    {
        try {
            $response = [
                ...
                'reminders' => [],
                'debug' => [
                    'session_direct_modal' => session('show_rating_modal_direct'),
                    'session_popup' => session('show_rating_popup'),
                    'session_service_data' => session('pending_rating_service'),
                    'all_session_keys' => array_keys(session()->all())
                ]
            ];

            // Check for direct modal trigger (new behavior)
            if (session('show_rating_modal_direct') && session('pending_rating_service')) {
            ...
            // Check for legacy popup trigger (fallback)
