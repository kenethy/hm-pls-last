            ...
            ->actions([
                ...
                Tables\Actions\Action::make('markAsCompleted')
                    ...
                    ->action(function (array $data, Service $record) {
                        ...
                        // 🎯 FILAMENT-NATIVE: Trigger rating action immediately
                        Notification::make()
                            ->title('✅ Servis Selesai')
                            ->body("Servis untuk {$record->customer_name} telah selesai.")
                            ->success()
                            ->actions([
                                \Filament\Notifications\Actions\Action::make('rate_mechanics')
                                    ->label('Rating Montir Sekarang')
                                    ->button()
                                    ->color('warning')
                                    ->action(function () use ($record) {
                                        // This will trigger the rating modal action
                                        return redirect()->to(request()->url() . '?show_rating=' . $record->id);
                                    })
                            ])
                            ->persistent()
                            ->send();
    ...
    public static function beforeSave(Forms\Form $form): void
    {
        ...
        if ($form->model->status === 'completed') {
            ...
            if ($form->model->mechanics()->count() === 0) {
                ...
                // 🚀 NEW APPROACH: Direct JavaScript injection for edit form completion
                Log::info('🎯 Service completed via edit form, injecting rating modal script for service: ' . $form->model->id);
                static::injectRatingModalScript($form->model);
