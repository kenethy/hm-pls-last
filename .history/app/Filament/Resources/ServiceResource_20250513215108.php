...
class ServiceResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ...
            ->mutateFormDataBeforeCreate(function (array $data): array { // Changed to mutateFormDataBeforeCreate
                // Check if we need to create a new vehicle
                $isNewVehicle = $data['is_new_vehicle'] ?? true; // Default to true if not set
                $vehicleId = $data['vehicle_id'] ?? null;
                $customerId = $data['customer_id'] ?? null;
                $carModel = $data['car_model'] ?? null;
                $licensePlate = $data['license_plate'] ?? null;

                // Only create a vehicle if it's marked as new, no existing vehicle selected,
                // and we have customer ID, car model, and license plate
                if ($isNewVehicle && !$vehicleId && $customerId && $carModel && $licensePlate) {
                    ...
                    if (!$existingVehicle) {
                        ...
                        $vehicle = Vehicle::create([
                            ...
                            'is_active' => true, // Assuming new vehicles are active
                            // Add any other required fields for vehicles
                        ]);
                        $data['vehicle_id'] = $vehicle->id; // Set the vehicle_id on the service data
                    } else {
                        // If vehicle with this license plate already exists, use it.
                        // You might want to update its customer_id if it's different, or handle as an error/warning.
                        // For now, just assign its ID.
                        $data['vehicle_id'] = $existingVehicle->id;
                        // If the existing vehicle belongs to a different customer, you might need specific logic.
                        // For example, if $existingVehicle->customer_id != $customerId, then decide what to do.
                        // Potentially update the vehicle's customer_id if that's desired:
                        // if ($existingVehicle->customer_id != $customerId) {
                        //    $existingVehicle->update(['customer_id' => $customerId]);
                        // }
                    }
                } elseif ($vehicleId) {
                    // If an existing vehicle was selected, ensure is_new_vehicle is false.
                    // This case should ideally be handled by afterStateUpdated on vehicle_id select.
                    // $data['is_new_vehicle'] = false; // This might be redundant if UI logic is correct
                }
                // Ensure vehicle_id is set if available, otherwise null
