                            ...
                            ->afterStateUpdated(function ($state, Forms\Set $set) {
                                if ($state) {
                                    ...
                                    if ($customer) {
                                        ...
                                        // Cek apakah pelanggan memiliki kendaraan
                                        $vehicles = Vehicle::where('customer_id', $customer->id)
                                            ->where('is_active', true)
                                            ->get();

                                        if ($vehicles->count() > 0) {
                                            // Tampilkan notifikasi dengan informasi kendaraan
                                            ...
                                            // Tampilkan notifikasi tanpa informasi kendaraan
                                            Notification::make()
                                                ->title('Pelanggan ditemukan')
                                                ->body("Data pelanggan {$customer->name} telah diisi otomatis. Silakan isi data kendaraan.")
                                                ->success()
                                                ->send();
                                        }
                                    } else {
                        ...
                        Forms\Components\Grid::make(2)
                            ->schema([
                                Forms\Components\TextInput::make('car_model')
                                    ...
                                    ->afterStateUpdated(function ($state, Forms\Set $set, Forms\Get $get) {
                                        if ($state && $get('vehicle_id')) {
                                            // Jika model mobil diubah dan ada kendaraan yang dipilih, reset vehicle_id
                                            $set('vehicle_id', null);
                                            $set('is_new_vehicle', true);

                                            Notification::make()
                                                ->title('Kendaraan Baru')
                                                ->body("Anda sedang membuat kendaraan baru dengan model: {$state}")
                                                ->info()
                                                ->send();
                                        }
                                    }),
                                ...
                                Forms\Components\TextInput::make('license_plate')
                                    ...
                                    ->afterStateUpdated(function ($state, Forms\Set $set, Forms\Get $get) {
                                        if ($state && $get('vehicle_id')) {
                                            // Jika nomor plat diubah dan ada kendaraan yang dipilih, reset vehicle_id
                                            $set('vehicle_id', null);
                                            $set('is_new_vehicle', true);

                                            Notification::make()
                                                ->title('Kendaraan Baru')
                                                ->body("Anda sedang membuat kendaraan baru dengan nomor plat: {$state}")
                                                ->info()
                                                ->send();
                                        }
                                    }),
    ...
    public static function beforeSave(Forms\Form $form): void
    {
        ...
        if ($form->model->phone && $form->model->customer_name) {
            ...
            if ($form->model->license_plate && $form->model->car_model) {
                // Selalu coba buat kendaraan baru jika ada license_plate dan car_model
                // Ini akan memastikan kendaraan baru dibuat jika plat nomor belum ada
                // atau menggunakan kendaraan yang sudah ada jika plat nomor sudah ada
