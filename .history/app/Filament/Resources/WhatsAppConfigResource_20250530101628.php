...
class WhatsAppConfigResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('refresh_qr_code')
                    ->label('QR Code Baru')
                    ->icon('heroicon-o-arrow-path')
                    ->color('warning')
                    ->requiresConfirmation()
                    ->modalHeading('Generate QR Code Baru')
                    ->modalDescription('Ini akan menghapus sesi yang ada dan membuat QR code baru. Lanjutkan?')
                    ->action(function (WhatsAppConfig $record) {
                        $service = new WhatsAppService();

                        // First logout to clear session
                        $logoutResult = $service->forceLogout();

                        // Then generate fresh QR
                        $result = $service->getQRCode(true);

                        if ($result['success']) {
                            $duration = $result['qr_duration'];
                            Notification::make()
                                ->title('QR Code Baru Berhasil Dibuat')
                                ->success()
                                ->body("QR Code baru berhasil dibuat. Berlaku selama {$duration} detik.")
                                ->actions([
                                    \Filament\Notifications\Actions\Action::make('view_fresh_qr')
                                        ->label('Lihat QR Code Baru')
                                        ->url($result['qr_url'])
                                        ->openUrlInNewTab(),
                                ])
                                ->send();
                            ...
                            Notification::make()
                                ->title('Gagal Membuat QR Code Baru')
                                ->danger()
                                ->body($result['message'])
                                ->send();
                        }
    ...
    {
        // Use direct IP access - simple and reliable solution
        // This bypasses all CDN/domain restrictions
        return 'http://45.32.116.20:3000';
    }
