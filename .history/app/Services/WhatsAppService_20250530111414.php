...
class WhatsAppService
{
     ...
     */
    public function getQRCode(bool $forceRefresh = false): array
    {
        if (!$this->isAvailable()) {
            return [
                'success' => false,
        ...
        try {
            // Use fresh login endpoint if force refresh is requested
            $endpoint = $forceRefresh ? '/app/login-fresh' : '/app/login';
            ...
            if ($response->successful()) {
                ...
                if (isset($data['results']['qr_link'])) {
                    $message = $forceRefresh
                        ? 'Fresh QR Code generated successfully'
                        : 'QR Code generated successfully';

                    // Fix QR URL for external access
                    $qrUrl = $this->fixQRCodeUrl($data['results']['qr_link']);
                    ...
                    return [
                        ...
                        'is_fresh' => $data['results']['fresh'] ?? false,
                        'data' => $data,
                    ];
                }
    ...
    /**
     * Force logout to clear session and enable fresh QR generation.
     */
    public function forceLogout(): array
    {
        ...
        try {
            $response = $this->makeApiRequest('GET', '/app/logout');

            if ($response->successful()) {
                Log::info('WhatsApp session cleared successfully', [
                    'config_id' => $this->config->id,
                ]);
