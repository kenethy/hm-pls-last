...
class Mechanic extends Model
{
    ...
    /**
     * Get the cumulative report for this mechanic.
     */
    public function cumulativeReport()
    {
        return $this->hasOne(MechanicReport::class)->where('is_cumulative', true);
    }
    ...
    {
        return $this->hasMany(MechanicReportArchive::class);
    }
    ...
    {
        return $this->hasMany(MechanicRating::class);
    }

    /**
     * Get average rating for this mechanic.
     */
    ...
    /**
     * Get total ratings count for this mechanic.
     */
    ...
    public function calculateWeeklyLaborCost($weekStart, $weekEnd)
    {
        ...
        // Hanya hitung biaya jasa dari servis yang berstatus 'completed'
    ...
    /**
     * Get or create cumulative report for this mechanic.
     */
    public function getOrCreateCumulativeReport()
    {
        $report = $this->cumulativeReport()->first();
    ...
    /**
     * Update cumulative report when a service is completed.
     */
    public function updateCumulativeReport($serviceId, $laborCost)
    {
        $report = $this->getOrCreateCumulativeReport();
     ...
     */
    public function calculateCumulativeStats()
    {
        return \Illuminate\Support\Facades\DB::table('mechanic_service')
            ->join('services', 'mechanic_service.service_id', '=', 'services.id')
            ->where('mechanic_service.mechanic_id', $this->id)
            ->where('services.status', 'completed')
            ->selectRaw('
                COUNT(*) as total_services,
                COALESCE(SUM(mechanic_service.labor_cost), 0) as total_labor_cost,
                MIN(services.completed_at) as first_service_date,
                MAX(services.completed_at) as last_service_date
            ')
            ->first();
    }
}
