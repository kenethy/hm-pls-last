...
# Tampilkan semua container yang berjalan untuk debugging
...
# Cari nama container yang benar - pastikan ini adalah container aplikasi Laravel
CONTAINER_NAME=$(docker ps | grep -E 'app|laravel|php' | grep -v 'phpmyadmin' | awk '{print $NF}' | head -1)

if [ -z "$CONTAINER_NAME" ]; then
    echo "Error: Tidak dapat menemukan container PHP/Laravel secara otomatis."
    echo "Silakan masukkan nama container Laravel/PHP Anda secara manual:"
    read -p "Nama container: " CONTAINER_NAME

    if [ -z "$CONTAINER_NAME" ]; then
        echo "Error: Nama container tidak boleh kosong. Silakan cek container yang berjalan dengan 'docker ps'."
        exit 1
    fi
fi
...
echo "Fitur riwayat servis montir juga telah diperbaiki dengan pendekatan yang lebih sederhana dan langsung."
...
echo "6. Implementasi dengan rute langsung untuk keandalan yang lebih baik"
echo ""
...
echo ""
echo "URL langsung untuk mengakses riwayat servis montir:"
echo "https://hartonomotor.xyz/admin/mechanic-reports/{id}/services"
