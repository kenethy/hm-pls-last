...
check_system_requirements() {
    ...
    # Check available disk space (minimum 2GB, recommended 5GB)
    AVAILABLE_SPACE=$(df / | awk 'NR==2 {print $4}')
    REQUIRED_SPACE=2097152   # 2GB in KB
    RECOMMENDED_SPACE=5242880 # 5GB in KB
    ...
    if [[ $AVAILABLE_SPACE -lt $REQUIRED_SPACE ]]; then
        ...
        log_warning "Low disk space: $(($AVAILABLE_SPACE/1024/1024))GB. Recommended: 5GB for comfortable operation"
    fi

    # Check memory (minimum 512MB, recommended 1GB)
    TOTAL_MEM=$(free -m | awk 'NR==2{print $2}')
    if [[ $TOTAL_MEM -lt 512 ]]; then
        error_exit "Insufficient memory: ${TOTAL_MEM}MB. Minimum required: 512MB"
    elif [[ $TOTAL_MEM -lt 1024 ]]; then
        log_warning "Low memory detected: ${TOTAL_MEM}MB. Recommended: 1GB+ for smooth operation"
    fi
