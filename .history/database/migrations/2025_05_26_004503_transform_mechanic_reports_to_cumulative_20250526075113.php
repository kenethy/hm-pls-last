...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Step 3: Modify the mechanic_reports table structure
        Schema::table('mechanic_reports', function (Blueprint $table) {
            // Remove weekly date constraints
            $table->dropUnique(['mechanic_id', 'week_start', 'week_end']);

            // Change week_start and week_end to period_start and period_end (nullable for cumulative)
            $table->renameColumn('week_start', 'period_start');
            $table->renameColumn('week_end', 'period_end');

            // Make period dates nullable for cumulative reports
            ...
            $table->unique(['mechanic_id', 'is_cumulative']);
        });
    ...
    public function down(): void
    {
        ...
        DB::statement('
            INSERT INTO mechanic_reports
            (mechanic_id, week_start, week_end, services_count, total_labor_cost, notes, is_paid, paid_at, created_at, updated_at)
            SELECT
                mechanic_id, week_start, week_end, services_count, total_labor_cost, notes, is_paid, paid_at, created_at, updated_at
            FROM mechanic_report_archives
