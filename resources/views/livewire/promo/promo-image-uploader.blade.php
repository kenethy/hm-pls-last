<div>
    <div class="space-y-4">
        @if($imagePath && $imageUrl)
            <div class="relative">
                <img src="{{ $imageUrl }}" alt="Preview" class="max-w-full h-auto max-h-64 rounded-lg border border-gray-200" />
                <button 
                    type="button" 
        ...
        @else
            <div class="space-y-2">
                <label for="image-{{ $this->id }}" class="inline-flex text-sm font-medium text-gray-700">Gambar Promo</label>
                <input 
                    type="file" 
                    id="image-{{ $this->id }}" 
                    wire:model="image" 
